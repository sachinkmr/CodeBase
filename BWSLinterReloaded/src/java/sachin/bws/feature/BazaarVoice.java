 ...
 * @author sku202
 */
public class BazaarVoice implements Featurable {
    ...
    public BazaarVoice(Site site) {
        this.site = site;
    }

    @Override
    public boolean hasFeature() {
        //ProductRatingsEnabled && EnableRating && ProductRatingsProvider
        try {
            return site.getConfigAsJSON().getString("ProductRatingsProvider").toLowerCase().contains("bazaarvoice") && site.getConfigAsJSON().getString("ProductRatingsEnabled").equalsIgnoreCase("true") && site.getConfigAsJSON().getString("EnableRating").equalsIgnoreCase("true");
    ...
    public JSONObject getData() {
        ...
        if (site.isMOS()) {
            try {
                result.put("hasErrors", true);
                errors.put("Error", "Feature is not applicable on MOS site");
                result.put("errors", errors);
            } catch (JSONException ex) {
                Logger.getLogger(BazaarVoice.class.getName()).log(Level.SEVERE, null, ex);
            }
        ...
        for (UrlLink link : links) {
            if (!hash.contains(link.getTemplateName())) {
                ...
                try {
                    ...
                    wait.until(ExpectedConditions.presenceOfElementLocated(By.className("bv-write-review")));
                    driver.findElement(By.className("bv-write-review")).click();
                    wait.until(ExpectedConditions.presenceOfElementLocated(By.className("bv-mbox-injection-container")));
                    WebElement element = driver.findElement(By.id("bv-radio-rating-5"));
                    ...
                    try {
                        ...
                        Logger.getLogger(BazaarVoice.class.getName()).log(Level.SEVERE, null, ex1);
                    }
                }
                if (e != null) {
                    try {
